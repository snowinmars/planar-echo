...
import styles from './MobilePhrase.module.scss';

type MobilePhraseProps = Readonly<{
  view: CurrentDialogueView;
  tlkSource: TlkSource;
  loading: boolean;
  selectResponse: DialogueStore['selectResponse'];
}>;
export const MobilePhrase: FC<MobilePhraseProps> = ({
  view,
  tlkSource,
  loading,
  selectResponse,
      ...
      <div className={styles.responses}>
        {
          view.responses.map(({ response, index, kind, marker }) => (
            <Button
              ...
              disabled={loading}
              onClick={() => {
                selectResponse(response, 'mobile').catch((error: unknown) => console.error(error));
              }}
            >
