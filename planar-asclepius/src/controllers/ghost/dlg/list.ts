...
import action from '@/services/ghost/dlg/list/action.js';
...
const routeConfig = (): RouteConfig => ({
  ...
  path: '/api/ghost/dlg',
  tags: ['ghostDlg'],
  description: 'Get available dlgs in ghost format',
  request: {
    body: {
      required: true,
      content: {
  ...
  responses: {
    200: {
      description: 'Available dlgs in ghost format',
      content: {
        'application/json': {
          schema: responseOk,
        },
    ...
    404: {
      description: 'Available dlgs are not found by this path',
      content: {
        'application/json': {
