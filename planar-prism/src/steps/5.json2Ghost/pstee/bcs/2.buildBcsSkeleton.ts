...
import type { RawBcs, RawBcsArg, RawBcsBlockScope } from '@/steps/4.biffs2json/pstee/bcs/index.js';
...
const writeArgFields = (writer: Writer, arg: RawBcsArg, offset: number): void => {
  switch (arg.kind) {
    case 'int':
...
const writeScopeBody = (writer: Writer, scope: RawBcsBlockScope, offset: number): void => {
...
const writeScope = (writer: Writer, scope: RawBcsBlockScope, offset: number): void => {
  writer.writeLine(`{`, offset);
  writeScopeBody(writer, scope, offset);
  writer.writeLine(`},`, offset);
};

const buildBcsSkeleton = (bcs: RawBcs): string => {
  const id = createLowercaseId(bcs.resourceName);
  ...
  for (const block of bcs.blocks) {
    ...
    for (const action of block.actions) writeScope(writer, action, 10);
    writer.writeLine(`],`, 8);
    writer.writeLine(`},`, 6);
  }
  writer.writeLine(`],`, 4);
...
export default buildBcsSkeleton;
