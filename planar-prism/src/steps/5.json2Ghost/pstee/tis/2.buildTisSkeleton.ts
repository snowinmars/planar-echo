...
import type { GhostTis, GhostTisPalette, GhostTisPvrz } from '@planar/shared';

const isTisPalette = (tis: GhostTis): tis is GhostTisPalette => tis.variant === 'palette';

const writeTisPalette = (tis: GhostTisPalette): string => {
  const id = withoutExtension(tis.resourceName);
...
const writeTisPvrz = (tis: GhostTisPvrz): string => {
  const id = withoutExtension(tis.resourceName);
  const writer = createWriter();
...
export const buildTisSkeleton = (tis: GhostTis): string => {
