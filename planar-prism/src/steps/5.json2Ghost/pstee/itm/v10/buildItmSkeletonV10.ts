...
const writeEffect = (writer: Writer, effect: GhostItmEffectV10): void => {
...
const writeAbility = (writer: Writer, ability: GhostItmAbilityV10): void => {
...
export const buildItmSkeletonV10 = (itm: GhostItmV10, discover: DiscoverNext): string => {
  const npcLowercaseId = createLowercaseId(itm.resourceName);

  discover({ type: 'itm', name: npcLowercaseId });

  const writer = createWriter();
  writer.writeLine(`import type { GhostItm } from '@planar/shared';`);
  writer.br();
  writer.writeLine('/**');
  writer.writeLine(` * Original source: ${itm.resourceName}`);
  writer.writeLine(' */');
  writer.writeLine(`const _${npcLowercaseId}ItmSkeleton = () => {`);
  writer.writeLine(`const itm: GhostItm = {`, 2);

  writer.writeLine(`resourceName: '${itm.resourceName.replaceAll(`'`, `\\'`)}',`, 4);
  writer.writeLine(`version: '${itm.version}',`, 4);
  writer.writeLine(`unidentifiedNameRef: ${itm.unidentifiedNameRef},`, 4);
  writer.writeLine(`identifiedNameRef: ${itm.identifiedNameRef},`, 4);
  writer.writeLine(`unidentifiedDescriptionRef: ${itm.unidentifiedDescriptionRef},`, 4);
  writer.writeLine(`identifiedDescriptionRef: ${itm.identifiedDescriptionRef},`, 4);
  writer.writeLine(`dropSound: '${itm.dropSound ?? null}',`, 4);
  writeFlags(
    writer,
    itm.flags,
    'flags',
  ...
  writer.writeLine(`category: '${itm.category}',`, 4);
  writeFlags(
    writer,
    itm.unusableBy,
    'unusableBy',
  ...
  writer.writeLine(`equippedAppearance: '${itm.equippedAppearance}',`, 4);
  writer.writeLine(`minLevel: ${itm.minLevel},`, 4);
  writer.writeLine(`minStrength: ${itm.minStrength},`, 4);
  writer.writeLine(`minStrengthBonus: ${itm.minStrengthBonus},`, 4);
  writeFlags(
    writer,
    itm.kitUsability1,
    'kitUsability1',
  ...
  writer.writeLine(`minIntelligence: ${itm.minIntelligence},`, 4);
  writeFlags(
    writer,
    itm.kitUsability2,
    'kitUsability2',
  ...
  writer.writeLine(`minDexterity: ${itm.minDexterity},`, 4);
  writeFlags(
    writer,
    itm.kitUsability3,
    'kitUsability3',
  ...
  writer.writeLine(`minWisdom: ${itm.minWisdom},`, 4);
  writeFlags(
    writer,
    itm.kitUsability4,
    'kitUsability4',
  ...
  writer.writeLine(`minConstitution: ${itm.minConstitution},`, 4);
  writer.writeLine(`weaponProficiency: '${itm.weaponProficiency}',`, 4);
  writer.writeLine(`minCharisma: ${itm.minCharisma},`, 4);
  writer.writeLine(`price: ${itm.price},`, 4);
  writer.writeLine(`maxInStack: ${itm.maxInStack},`, 4);
  writer.writeLine(`inventoryIcon: '${itm.inventoryIcon}',`, 4);
  writer.writeLine(`loreToId: ${itm.loreToId},`, 4);
  writer.writeLine(`groundIcon: '${itm.groundIcon}',`, 4);
  writer.writeLine(`weight: ${itm.weight},`, 4);
  writer.writeLine(`pickupSound: '${itm.pickupSound}',`, 4);
  writer.writeLine(`enchantment: ${itm.enchantment},`, 4);
  ...
  writer.writeLine('};', 2);
  writer.writeLine('return itm;', 2);
  writer.writeLine('};');
  writer.writeLine(`export default _${npcLowercaseId}ItmSkeleton;`);
