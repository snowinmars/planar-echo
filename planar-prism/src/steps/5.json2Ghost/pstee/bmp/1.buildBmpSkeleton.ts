...
import { isBmpV1 } from '@/steps/4.biffs2json/pstee/bmp/index.js';

import type { RawBmp, RawBmpV1, RawBmpV5 } from '@/steps/4.biffs2json/pstee/bmp/index.js';

const writeBmpV1 = (bmp: RawBmpV1): string => {
  const writer = createWriter();
  const id = bmp.resourceName.split('.')[0]!;
...
const writeBmpV5 = (bmp: RawBmpV5): string => {
  const writer = createWriter();
  const id = bmp.resourceName.split('.')[0]!;
...
export const buildBmpSkeleton = (bmp: RawBmp): string => {
  if (isBmpV1(bmp)) return writeBmpV1(bmp);
  return writeBmpV5(bmp);
};
