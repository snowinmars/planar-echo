...
const isMosV1 = (mos: RawMos): mos is RawMosV1 => mos.variant === 'v1';

const writeMosV1 = (writer: Writer, mos: RawMosV1): void => {
  writer.writeLine(`import type { GhostMosV1 } from '@planar/shared';`);
  writer.br();
  writer.writeLine('/**');
  writer.writeLine(` * Original source: ${mos.resourceName}`);
  writer.writeLine(' */');
  const id = createLowercaseId(mos.resourceName);
  ...
  writer.writeLine(`signature: 'mos',`, 4);
...
const writeMosV2 = (writer: Writer, mos: RawMosV2): void => {
...
const buildMosSkeleton = (mos: RawMos): string => {
  const writer = createWriter();
  if (isMosV1(mos)) writeMosV1(writer, mos);
  else writeMosV2(writer, mos);
  return writer.done();
};

export default buildMosSkeleton;
