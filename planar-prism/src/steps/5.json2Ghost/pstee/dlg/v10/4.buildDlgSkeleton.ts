...
type BuildDlgSkeletonProps = Readonly<{
  dlg: NestedDlg;
  npcId: string;
  npcNameRef: number;
  discover: DiscoverNext;
}>;
export const buildDlgSkeleton = ({
  dlg,
  npcId,
  npcNameRef,
  discover,
  ...
  const writer = createWriter();
  writer.writeLine(`import { registerDlg } from '@planar/shared';`);
  writer.writeLine(`import type { DlgLogic } from '@planar/shared';`);
  writer.br();
  writer.writeLine('/**');
  writer.writeLine(` * Original source: ${dlg.resourceName}`);
  writer.writeLine(' */');
  writer.writeLine(`const _whoRef = ${npcNameRef};`);
  writer.writeLine(`const ${npcLowercaseId}DlgSkeleton = (dlgLogic: DlgLogic) => {`);
  writer.writeLine(`const dlg = registerDlg<DlgLogic>('${dlg.resourceName.replaceAll(`'`, `\\'`)}', dlgLogic);`, 2);
  writer.br();
  ...
  for (const state of dlg.states) {
    ...
    for (const response of state.responses) {
      ...
      const targetState = response.flags.includes('terminates dialog')
        ? 'terminate dlg'
        : formStateId(just(response.nextDlg).replace(`'`, ``), just(response.nextDlgState));
  ...
  writer.writeLine('return dlg.expose();', 2);
  writer.writeLine('};');
  writer.writeLine(`export default ${npcLowercaseId}DlgSkeleton;`);
