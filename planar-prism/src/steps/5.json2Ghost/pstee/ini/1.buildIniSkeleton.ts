...
import type { RawIniCreatureIniSection } from '@/steps/4.biffs2json/pstee/ini/v1/parsers/parseCreatureSectionV1.types.js';
import type { RawIniCreatureIniScopedVariable } from '@/steps/4.biffs2json/pstee/ini/v1/parsers/parseScopedVariableV1.types.js';
...
const writeScopedVar = (writer: Writer, propertyName: string, value: RawIniCreatureIniScopedVariable, offset: number): void => {
...
const writeCreatureSection = (writer: Writer, section: RawIniCreatureIniSection): void => {
  writer.writeLine(`{`, 6);
...
export const buildIniSkeleton = (ini: RawIni): string => {
  const writer = createWriter();
  const id = ini.resourceName.split('.')[0]!;
