...
/* createGenerator().register().flags('spawnMethodV10', {
 *   byte1:['do not spawn if 0x4 is set','one time spawn','spawn temporarily disabled',],
 * }).write();
 */
const spawnMethodV10 = {
  // byte1
  0x1: 'do not spawn if 0x4 is set',
  0x2: 'one time spawn',
  0x4: 'spawn temporarily disabled',
  // 0x8: unused
  // 0x10: unused
  // 0x20: unused
  // 0x40: unused
...
type SpawnMethodV10 = typeof spawnMethodV10[keyof typeof spawnMethodV10];
...
export const extendMap = {
  spawnMethod: extend(spawnMethodV10),
  presentedAtFlags: extend(presentedAtFlagsV10),
};

export type RawAreSpawnPointV10 = Readonly<{
  name: string;
  at: Point;
  creatures: string[];
  creaturesCount: number;
  encounterDifficulty: number; // Base creature number to spawn. The actual number to spawn is given by: `(Frequency * Average Party Level) / Creature Power`; With results rounded down.
  spawnRate: number;
  method: SpawnMethodV10[];
  duration: number;
