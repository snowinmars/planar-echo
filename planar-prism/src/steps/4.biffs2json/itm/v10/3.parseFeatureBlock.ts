import { offsetMap } from '../v10.types/3.featureBlock.js';

import type { BufferReader } from '../../../../pipes/readers.js';
import type { FeatureBlockV10 } from '../v10.types/3.featureBlock.js';

const parseFeatureBlock = (reader: BufferReader): FeatureBlockV10 => {
  ...
  const dispelOrResistance = reader.map.byte(offsetMap.resistance.parse);
...
const parseFeatureBlocks = (reader: BufferReader, count: number): FeatureBlockV10[] => {
  const featureBlockSize = 48;
  return Array.from<never, FeatureBlockV10>({ length: count }, (_, i) => parseFeatureBlock(reader.fork(reader.offset + i * featureBlockSize)));
};
