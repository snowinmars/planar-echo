...
const parseFeatureBlocks = (reader: BufferReader, count: number): FeatureBlockV11[] => {
  const featureBlockSize = 48;
  return Array.from<never, FeatureBlockV11>({ length: count }, (_, i) => parseFeatureBlock(reader.fork(reader.offset + i * featureBlockSize)));
};
