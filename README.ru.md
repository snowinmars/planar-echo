...
- **Конвертация** из оригинальных бинарников в JSON, бинарные ассеты (PNG/WAV) и модули Ghost: `.acm`, `.are`, `.bam`, `.bcs`, `.bmp`, `.cre`, `.dlg`, `.eff`, `.ids`, `.ini`, `.itm`, `.mos`, `.mus`, `.pvrz`, `.tis`, `.tlk`, `.wav`, `.wed`.
...
| Сервис               | Задача                                                                                     |
| -------------------- | ------------------------------------------------------------------------------------------ |
| **planar-prism**     | CLI: BIFF → JSON → ассеты (PNG/WAV) → Ghost TypeScript; сам по себе или как дочерний fork-процесс. |
...
3. Prism запишет JSON, ассеты и Ghost в каталог ghost; прогресс отображается в UI.
