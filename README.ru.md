...
![Пример текущего состояния диалога](./_dev/promo_dlg.png)
![Пример текущего состояния существа](./_dev/promo_cre.png)
![Пример текущего состояния предмета](./_dev/promo_itm.png)
...
- **Конвертация** из оригинальных бинарников в JSON и модули Ghost: `.cre`, `.dlg`, `.eff`, `.ids`, `.ini`, `.itm`, `.tlk`.
...
- Проверка сообществом UI-локалей cs, de, fr, ko, pl.
- Более глубокая логика скриптов.
- Упаковка JSON обратно в BIFF для моддинга оригинала.
...
| **planar-shell**     | React + Zustand + MUI: мастер конвертации, настройки, просмотр диалогов/существ/предметов. |
...
4. Открой в Shell `/dialogue`, `/creature` или `/item`.
