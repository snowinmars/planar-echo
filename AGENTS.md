...
6. Просмотр: Shell → REST `/api/ghost/*` + `@planar/shared` `dlgEngine` / shell `engine/dlgLogic.ts`.
...
**REST** (`planar-asclepius/src/controllers/router.ts`) - типично JSON body; ghost: `/api/ghost/cre|dlg|itm|tlk`, map: `/api/map/creToDlgs|dlgToCre|itmToDlgs|dlgToItm`. Схема: `planar-asclepius/src/swagger/swagger.json`.
...
- Текстовые и бинарные артефакты, not a single text Domain-Specific Language: промежуточный **JSON** и **TS** на диске; runtime для движка - **bundled JS** под `planar-ghost/ghost/{cre,dlg,itm,...}/dist`.
...
- `src/ghost/` - внешний контракт ghostDir: `GhostCre*`, `GhostDlg*`, `GhostItm*`, `GhostTlk`
...
- `router/router.tsx` - маршруты: `/`, `/details`, `/convert`, `/dlg`, `/cre`, `/itm`, `/settings`, `/stores`
- `components/Convert/` - мастер конверсии; шаг 6 - WS на `{backendUrl}/api/prism/index` (`store/step6.ts`)
- `components/runners/` - Dlg, Cre, Itm; `dialogueResolution/`
